...
`apps/api/migrations/001_initial.sql` creates tables and search indexes; `002_demo_seed.sql` inserts synthetic records; `003_operational_constraints.sql` adds uniqueness and search indexes. `npm run db:migrate` applies all ordered SQL migrations and `npm run db:seed` loads the demo fixture. Docker Compose waits for PostgreSQL health, applies migrations and seeds before starting the API. The API also has an in-memory fallback so the UI can be previewed without PostgreSQL.
